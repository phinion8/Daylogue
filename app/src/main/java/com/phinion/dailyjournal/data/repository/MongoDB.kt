package com.phinion.dailyjournal.data.repository
...
//This class will help to use realm sync
object MongoDB : MongoRepository {
        ...
        if (user != null) {
            val config = SyncConfiguration.Builder(user, setOf(Diary::class))
                .initialSubscriptions { sub ->
                    ...
                    //Here we are subscribing to all diaries that contains the ownerId of the current authenticated user
    ...
    override suspend fun updateDiary(diary: Diary): RequestState<Diary> {
        return if (user != null) {
            ...
            realm.write {
                val queriesDiary = query<Diary>(query = "_id == $0", diary._id).first().find()
                if (queriesDiary != null) {
                    queriesDiary.title = diary.title
                    queriesDiary.description = diary.description
                    queriesDiary.mood = diary.mood
                    queriesDiary.images = diary.images
                    queriesDiary.date = diary.date
                    RequestState.Success(data = queriesDiary)
                } else {
    ...
    override fun getFilteredDiaries(zonedDateTime: ZonedDateTime): Flow<Diaries> {
        return if (user != null) {
            ...
            try {
                ...
                realm.query<Diary>(
                    //here this query means that first we are going to the first parameter and then second parameter and than the third parameter
